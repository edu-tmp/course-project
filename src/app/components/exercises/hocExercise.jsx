...
const withFunctions = (Component) => (props) => {
  ...
  return (
    <CardWrapper>
      <Component isAuth={isAuth} onLogOut={handleLogout} onLogin={handleLogin} {...props} />
    </CardWrapper>
  );
};
// Component with HOC
  ...
  return (
    <CollapseWrapper title="Упражнение">
      ...
      <ul>
        ...
        <li>
          Передает параметр <code>isAuth</code>, который является результатом проверки наличия записи с названием{' '}
          <code>user</code> в <code>localStorage</code>
        </li>
        <li>
          Передает параметр <code>onLogin</code> и <code>onLogOut</code> для управления записью с названием{' '}
          <code>user</code> в <code>localStorage</code>
        </li>
      </ul>
      <Divider />
